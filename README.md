...
모든 메시지는 **보낸 시각**으로 시작하고 그 아래 머리말이 붙는다. 머리말은 `config.json` 의
`message_header` 로 바꾼다. 시각이 있으면 반복 알림 열 통이 같은 내용이어도 구분이 되고,
나중에 스크롤을 올려 봐도 언제 온 알림인지 바로 안다.
