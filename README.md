...
```
(9/22(화), 22:00) 준규의 용아맥 오디세이 연애💑프로젝트
...
```
(9/12(금), 21:30) 준규의 용아맥 오디세이 연애💑프로젝트
...
첫 줄이 <b>(상영 날짜, 시작 시각) 머리말</b> 형태다. 폰 잠금화면 팝업은 첫 줄만 보여 주므로,
**열어 보지 않고도 어느 회차 표인지** 알 수 있어야 하기 때문이다. 회차가 여럿이면
`(9/22(화), 22:00 외 2건)` 처럼 붙는다. 머리말은 `config.json` 의 `message_header` 로 바꾼다.

`/status` 같은 명령 응답에는 회차가 없으므로 머리말만 붙는다.
